...
def test_confirm_contract_notifies_hr(integration_client):
    ...
    with session_factory() as db:
        ...
        assert len(notes) == 1
        assert "xác nhận" in notes[0].title.lower() or "Xác nhận" in notes[0].title
        assert "Intern User" in notes[0].body
